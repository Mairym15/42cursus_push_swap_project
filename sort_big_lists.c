...
/*   Updated: 2023/02/25 21:59:09 by mkerkeni         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
t_lst	*sort_list(t_lst *lst, t_var v, int move)
{
	...
	while ((lst->b) != NULL)
	{
		...
		lst = match_best_values(lst, v.best_a, v.best_b);
	}
	return (lst);
}

static t_lst	*put_back_in_place_list(t_lst *lst, int bool)
{
	if (bool == 1)
	{
		while ((lst->a)->index != 0)
...
static t_lst	*fill_b(t_lst *lst, t_var v)
{
...
t_lst	*sort_big_lists(t_lst *lst)
{
	...
	lst = sort_list(lst, v, move);
	middle = get_middle(&lst->a);
	if (check_position(&lst->a, 0, middle) == 1)
		lst = put_back_in_place_list(lst, 1);
	else
		lst = put_back_in_place_list(lst, 0);
	return (lst);
}
